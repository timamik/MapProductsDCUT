        ...
        data.forEach(function(org) {
            org.locations.forEach(function(loc) {
                ...
                var marker = L.circleMarker([loc.latitude, loc.longitude], {
                    ...
                    .bindPopup(`<a href="timamik.github.io/MapProductsDCUT/table.html?id=${encodeURIComponent(org.name)}" target="_blank"><b>${org.name}</b></a><br>${loc.description1}: шт. <br>${description2.toLocaleString('ru-RU', { style: 'currency', currency :'RUB'})}<br>${loc.address}</br>`);
